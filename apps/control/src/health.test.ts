...
describe("tenant health checks", () => {
  it("requires fresh checks and exact release alignment for portfolio readiness", () => {
    ...
    expect(result.health).toEqual({ published: 3, ready: 1, failing: 1, stale: 1, unchecked: 0, lastCheckedAt: "2026-08-05T06:47:00.000Z" });
  ...
  it("requires an exact live hostname and release match", async () => {
    ...
    expect(result).toMatchObject({ checked: 3, ready: 1, notReady: 1, unreachable: 1, truncated: false });
