...
export async function rollupDate(env: Env, metricDate: string, now = new Date()): Promise<RollupResult> {
  ...
  try {
    const expectedRows = await env.DB.prepare(
      "SELECT d.id AS domain_id,COUNT(DISTINCT CAST((unixepoch(h.checked_at)-2820)/21600 AS INTEGER)) AS expected_canaries FROM domains d LEFT JOIN tenant_health_checks h ON h.domain_id=d.id AND h.check_source='scheduled' AND h.checked_at>=? AND h.checked_at<? WHERE d.lifecycle_status='published' GROUP BY d.id",
    ...
    const canariesSql = `SELECT index1 AS domain_id, toDate(timestamp) AS metric_date, count(DISTINCT toStartOfInterval(timestamp - INTERVAL '47' MINUTE, INTERVAL '6' HOUR)) AS observed_canaries, max(_sample_interval) AS max_sample_interval FROM ${env.ANALYTICS_DATASET} WHERE ${where} AND blob1 = 'health_canary' AND blob8 = 'health_scheduled' AND blob7 != '' GROUP BY index1, metric_date`;
