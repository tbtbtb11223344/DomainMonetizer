...
async function compileRelease(
  ...
  const enabledOffer = await env.DB.prepare(
    "SELECT 1 AS found FROM routing_policies rp JOIN offers o ON o.id=rp.offer_id LEFT JOIN affiliate_campaigns ac ON ac.id=rp.campaign_id AND ac.offer_id=o.id WHERE rp.status='active' AND o.status='active' AND (rp.domain_id=? OR rp.domain_id IS NULL) AND (rp.vertical IS NULL OR rp.vertical=?) AND (rp.country IS NULL OR rp.country=?) AND (rp.starts_at IS NULL OR rp.starts_at<=?) AND (rp.ends_at IS NULL OR rp.ends_at>?) AND (rp.campaign_id IS NULL OR ac.status='active') AND (lower(o.provider)<>'marketcall' OR ac.status='active') LIMIT 1",
  )
...
export function mountApi(app: App): void {
  ...
  app.get("/api/metrics/overview", async (c) => {
    ...
    const monetization = {
      ...
      pendingCalls: Number(monetizationState?.provider_pending_calls ?? 0),
      unsuccessfulCalls: Number(monetizationState?.provider_unsuccessful_calls ?? 0),
