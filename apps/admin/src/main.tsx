...
function App() {
  ...
  const primaryKpiLabel = exactUniqueKpiPending
    ? sampledEstimateAvailable
      ? `Sampled ${overview?.sampledMetricDate} · max ×${overview?.totals.sampledUniqueSampleInterval ?? 1}`
  ...
  return <div className="shell">
    ...
    <main className="workspace" id={view}>
      <header className="topbar">
        ...
        {view === "portfolio" && <div className="summary"><div><strong>{domains.length}</strong><span>in system</span></div><div><strong>{totals.live}/{overview?.health.ready ?? "—"}</strong><span>live / ready</span></div><div><strong>{sampledEstimateAvailable ? `≈${formatNumber(overview?.totals.sampledUsUniqueVisitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(overview?.totals.usUniqueVisitors ?? 0)}</strong><span>{sampledEstimateAvailable ? "sampled U.S. estimate" : "U.S. qualified uniques"}</span></div><div><strong>{overview?.decisionGradeDays ?? 0}</strong><span>decision-grade days</span></div></div>}
      ...
      {view === "portfolio" && <>
      {overview && exactUniqueKpiPending && <div className="info" role="status"><span>U.S. unique KPI</span>{sampledEstimateAvailable ? `Showing sampled estimates from ${overview.sampledMetricDate} (maximum sampling interval ×${overview.totals.sampledUniqueSampleInterval}). ` : ""}Exact daily measurement starts {overview.exactSessionStartDate} UTC. Estimates are visible now but remain excluded from decision-grade evidence.</div>}
      ...
      <div className="content-grid">
        <section className="table-wrap" aria-busy={loading}>
          <table>
            ...
            <tbody>{domains.map((domain) => {
              ...
              <td><strong>{sampledEstimateAvailable ? `≈${formatNumber(metric?.sampled_us_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(metric?.us_unique_visitors ?? 0)}</strong><small>{sampledEstimateAvailable ? `${overview?.sampledMetricDate} sampled · ${formatPercent(metric?.sampled_us_unique_visitors ?? 0, metric?.sampled_unique_visitors ?? 0)} of ≈${formatNumber(metric?.sampled_unique_visitors ?? 0)}` : exactUniqueKpiPending ? `Exact daily KPI starts ${overview?.exactSessionStartDate ?? "after cutover"} UTC` : `${formatPercent(metric?.us_unique_visitors ?? 0, metric?.unique_visitors ?? 0)} of ${formatNumber(metric?.unique_visitors ?? 0)} qualified uniques`}</small></td>
        ...
        <aside className="inspector" aria-live="polite">
          {current ? <>
            ...
            <div className="signals"><div className="signals-head"><h3>Primary audience KPI</h3><span className={`evidence-state ${overview?.evidenceStatus ?? "collecting"}`}>{primaryKpiLabel}</span></div><dl><div><dt>{sampledEstimateAvailable ? "U.S. qualified estimate" : "U.S. qualified uniques"}</dt><dd>{sampledEstimateAvailable ? `≈${formatNumber(currentMetric?.sampled_us_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(currentMetric?.us_unique_visitors ?? 0)}</dd></div><div><dt>{sampledEstimateAvailable ? "All qualified estimate" : "All qualified uniques"}</dt><dd>{sampledEstimateAvailable ? `≈${formatNumber(currentMetric?.sampled_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(currentMetric?.unique_visitors ?? 0)}</dd></div><div><dt>U.S. % of uniques</dt><dd>{sampledEstimateAvailable ? formatPercent(currentMetric?.sampled_us_unique_visitors ?? 0, currentMetric?.sampled_unique_visitors ?? 0) : exactUniqueKpiPending ? "—" : formatPercent(currentMetric?.us_unique_visitors ?? 0, currentMetric?.unique_visitors ?? 0)}</dd></div><div><dt>Engagement</dt><dd>{formatPercent(currentMetric?.human_engaged_visits ?? 0, currentMetric?.likely_human_views ?? 0)}</dd></div><div><dt>Event pipeline</dt><dd>{!overview?.expectedFullDays ? "Starts after day one" : currentTelemetry?.verified ? "Verified" : "Unverified"}</dd></div></dl><p>{sampledEstimateAvailable ? `The ${overview?.sampledMetricDate} figures are sampling-adjusted estimates (maximum ×${currentMetric?.sampled_unique_sample_interval ?? 1}), not exact uniques.` : exactUniqueKpiPending ? `No sampled qualified-session estimate is available yet.` : "One qualified browser counts once per UTC day. Bots, previews, readiness checks, and excluded operator traffic do not count."} {exactUniqueKpiPending ? "The new exact daily-unique stream starts" : "Clean daily-unique measurement began"} {overview?.exactSessionStartDate ?? "—"} UTC. {overview?.rollupCoverageComplete ? `Coverage is complete through ${overview.rollupThrough ?? "the first completed day"}.` : `Coverage is ${overview?.observedFullDays ?? 0}/${overview?.expectedFullDays ?? 0} completed days; recovery targets ${overview?.latestCompletedDate ?? "the latest completed day"}.`} {samplingLabel}</p></div>
