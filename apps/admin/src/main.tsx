        ...
        <section className="table-wrap" aria-busy={loading}>
          <table>
            ...
            <tbody>{domains.map((domain) => {
              ...
              <td><strong>{formatNumber(metric?.us_unique_visitors ?? 0)}</strong><small>{formatPercent(metric?.us_unique_visitors ?? 0, metric?.unique_visitors ?? 0)} of {formatNumber(metric?.unique_visitors ?? 0)} human uniques</small></td>
