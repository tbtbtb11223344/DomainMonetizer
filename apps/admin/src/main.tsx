...
function App() {
  ...
  return <div className="shell">
    ...
    <main className="workspace" id={view}>
      ...
      {overview && economicPilot && <div className="info" role="status"><span>Economic pilot</span>{overview.monetization.activeRoutingPolicies} approved Marketcall route{overview.monetization.activeRoutingPolicies === 1 ? " is" : "s are"} live across {overview.monetization.activeCampaigns} active campaign{overview.monetization.activeCampaigns === 1 ? "" : "s"}. Tracked so far: {overview.monetization.uniquePhoneActions} unique likely-human phone action{overview.monetization.uniquePhoneActions === 1 ? "" : "s"}, {overview.monetization.phoneActions} total phone CTA action{overview.monetization.phoneActions === 1 ? "" : "s"}, and {overview.monetization.providerConfirmedCalls} provider-confirmed call{overview.monetization.providerConfirmedCalls === 1 ? "" : "s"}.</div>}
      ...
      <div className="content-grid">
        ...
        <aside className="inspector" aria-live="polite">
          {current ? <>
            ...
            {economicPilot && <div className="signals"><div className="signals-head"><h3>Phone action funnel</h3><span className="evidence-state collecting">Exact ledger</span></div><dl><div><dt>Unique phone actions</dt><dd>{formatNumber(currentMetric?.unique_phone_actions ?? 0)}</dd></div><div><dt>Total phone CTA actions</dt><dd>{formatNumber(currentMetric?.phone_actions ?? 0)}</dd></div><div><dt>Provider-confirmed calls</dt><dd>{formatNumber(currentMetric?.provider_confirmed_calls ?? 0)}</dd></div></dl><p>A unique phone action counts one likely-human browser per website per UTC day. It records the click-to-call handoff, not proof that a call connected; provider-confirmed calls remain a separate outcome.</p></div>}
