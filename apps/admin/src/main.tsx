...
function App() {
  ...
  const portfolioLatestEstimateSampled = (overview?.totals.sampledUniqueSampleInterval ?? 1) > 1;
  const currentLatestEstimateSampled = (currentMetric?.sampled_unique_sample_interval ?? 1) > 1;
  const primaryKpiLabel = exactUniqueKpiPending
    ? sampledEstimateAvailable
      ? `Latest ${overview?.sampledMetricDate} · max ×${overview?.totals.sampledUniqueSampleInterval ?? 1}`
  ...
  return <div className="shell">
    ...
    <main className="workspace" id={view}>
      <header className="topbar">
        ...
        {view === "portfolio" && <div className="summary"><div><strong>{domains.length}</strong><span>in system</span></div><div><strong>{totals.live}/{overview?.health.ready ?? "—"}</strong><span>live / ready</span></div><div><strong>{sampledEstimateAvailable ? `${portfolioLatestEstimateSampled ? "≈" : ""}${formatNumber(overview?.totals.sampledUsUniqueVisitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(overview?.totals.usUniqueVisitors ?? 0)}</strong><span>{sampledEstimateAvailable ? "latest-day U.S. qualified" : "U.S. qualified uniques"}</span></div><div><strong>{overview?.decisionGradeDays ?? 0}</strong><span>decision-grade days</span></div></div>}
      ...
      {overview && economicPilot && <div className="info" role="status"><span>Economic pilot</span>{overview.monetization.activeRoutingPolicies} approved Marketcall route{overview.monetization.activeRoutingPolicies === 1 ? " is" : "s are"} live across {overview.monetization.activeCampaigns} active campaign{overview.monetization.activeCampaigns === 1 ? "" : "s"}. {overview.monetization.validVisitorCallRequests} U.S. call request{overview.monetization.validVisitorCallRequests === 1 ? "" : "s"} carried a valid visitor cookie. Marketcall reconciliation: {overview.monetization.providerRecordedCalls} recorded, {overview.monetization.qualifiedCalls} qualified, {overview.monetization.pendingCalls} pending, and {overview.monetization.unsuccessfulCalls} unsuccessful. Cookieless and operator-excluded requests do not count.</div>}
      ...
      <div className="content-grid">
        <section className="table-wrap" aria-busy={loading}>
          <table>
            <thead><tr><th>Hostname</th><th>Vertical</th><th>{sampledEstimateAvailable ? "U.S. qualified estimate" : "U.S. qualified uniques"}</th><th>Parking baseline</th><th>Status</th><th aria-label="Open" /></tr></thead>
            <tbody>{domains.map((domain) => {
              ...
              const latestEstimateSampled = (metric?.sampled_unique_sample_interval ?? 1) > 1;
              ...
              <td><strong>{sampledEstimateAvailable ? `${latestEstimateSampled ? "≈" : ""}${formatNumber(metric?.sampled_us_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(metric?.us_unique_visitors ?? 0)}</strong><small>{sampledEstimateAvailable ? `${overview?.sampledMetricDate} ${latestEstimateSampled ? `sampled ×${metric?.sampled_unique_sample_interval ?? 1}` : "unsampled"} · ${formatPercent(metric?.sampled_us_unique_visitors ?? 0, metric?.sampled_unique_visitors ?? 0)} of ${latestEstimateSampled ? "≈" : ""}${formatNumber(metric?.sampled_unique_visitors ?? 0)}` : exactUniqueKpiPending ? `Exact daily KPI starts ${overview?.exactSessionStartDate ?? "after cutover"} UTC` : `${formatPercent(metric?.us_unique_visitors ?? 0, metric?.unique_visitors ?? 0)} of ${formatNumber(metric?.unique_visitors ?? 0)} qualified uniques`}</small></td>
        ...
        <aside className="inspector" aria-live="polite">
          {current ? <>
            ...
            <div className="signals"><div className="signals-head"><h3>Primary audience KPI</h3><span className={`evidence-state ${overview?.evidenceStatus ?? "collecting"}`}>{primaryKpiLabel}</span></div><dl><div><dt>{sampledEstimateAvailable ? "U.S. qualified latest day" : "U.S. qualified uniques"}</dt><dd>{sampledEstimateAvailable ? `${currentLatestEstimateSampled ? "≈" : ""}${formatNumber(currentMetric?.sampled_us_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(currentMetric?.us_unique_visitors ?? 0)}</dd></div><div><dt>{sampledEstimateAvailable ? "All qualified latest day" : "All qualified uniques"}</dt><dd>{sampledEstimateAvailable ? `${currentLatestEstimateSampled ? "≈" : ""}${formatNumber(currentMetric?.sampled_unique_visitors ?? 0)}` : exactUniqueKpiPending ? "—" : formatNumber(currentMetric?.unique_visitors ?? 0)}</dd></div><div><dt>U.S. % of uniques</dt><dd>{sampledEstimateAvailable ? formatPercent(currentMetric?.sampled_us_unique_visitors ?? 0, currentMetric?.sampled_unique_visitors ?? 0) : exactUniqueKpiPending ? "—" : formatPercent(currentMetric?.us_unique_visitors ?? 0, currentMetric?.unique_visitors ?? 0)}</dd></div><div><dt>Engagement</dt><dd>{formatPercent(currentMetric?.human_engaged_visits ?? 0, currentMetric?.likely_human_views ?? 0)}</dd></div><div><dt>Event pipeline</dt><dd>{!overview?.expectedFullDays ? "Starts after day one" : currentTelemetry?.verified ? "Verified" : "Unverified"}</dd></div></dl><p>{sampledEstimateAvailable ? `The ${overview?.sampledMetricDate} result is ${currentLatestEstimateSampled ? `sampling-adjusted at ×${currentMetric?.sampled_unique_sample_interval ?? 1}` : "unsampled for this domain"}, but predates the new exact-evidence boundary.` : exactUniqueKpiPending ? `No completed-day qualified-session result is available yet.` : "One qualified browser counts once per UTC day. Bots, previews, readiness checks, and excluded operator traffic do not count."} {exactUniqueKpiPending ? "The new exact daily-unique stream starts" : "Clean daily-unique measurement began"} {overview?.exactSessionStartDate ?? "—"} UTC. {overview?.rollupCoverageComplete ? `Coverage is complete through ${overview.rollupThrough ?? "the first completed day"}.` : `Coverage is ${overview?.observedFullDays ?? 0}/${overview?.expectedFullDays ?? 0} completed days; recovery targets ${overview?.latestCompletedDate ?? "the latest completed day"}.`} {samplingLabel}</p></div>
            {economicPilot && <div className="signals"><div className="signals-head"><h3>Call requests and outcomes</h3><span className="evidence-state collecting">Cookie + provider ledgers</span></div><dl><div><dt>Valid U.S. call requests</dt><dd>{formatNumber(currentMetric?.valid_visitor_call_requests ?? 0)}</dd></div><div><dt>Marketcall recorded</dt><dd>{formatNumber(currentMetric?.provider_recorded_calls ?? 0)}</dd></div><div><dt>Qualified calls</dt><dd>{formatNumber(currentMetric?.provider_confirmed_calls ?? 0)}</dd></div><div><dt>Pending review</dt><dd>{formatNumber(currentMetric?.provider_pending_calls ?? 0)}</dd></div><div><dt>Unsuccessful</dt><dd>{formatNumber(currentMetric?.provider_unsuccessful_calls ?? 0)}</dd></div></dl><p>A request counts only when the U.S. phone handoff carries a valid first-party visitor cookie. Marketcall outcomes remain separate; shared-number calls without exact attribution appear only in the portfolio total.</p></div>}
