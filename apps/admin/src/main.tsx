...
function App() {
  ...
  return <div className="shell">
    ...
    <main className="workspace" id={view}>
      ...
      {overview && economicPilot && <div className="info" role="status"><span>Economic pilot</span>{overview.monetization.activeRoutingPolicies} approved Marketcall route{overview.monetization.activeRoutingPolicies === 1 ? " is" : "s are"} live across {overview.monetization.activeCampaigns} active campaign{overview.monetization.activeCampaigns === 1 ? "" : "s"}. Marketcall is the call source of truth: {overview.monetization.providerRecordedCalls} provider-recorded call{overview.monetization.providerRecordedCalls === 1 ? "" : "s"}, {overview.monetization.qualifiedCalls} qualified, {overview.monetization.pendingCalls} pending, and {overview.monetization.unsuccessfulCalls} unsuccessful. CTA handoffs are excluded.</div>}
      ...
      <div className="content-grid">
        ...
        <aside className="inspector" aria-live="polite">
          {current ? <>
            ...
            {economicPilot && <div className="signals"><div className="signals-head"><h3>Marketcall outcomes</h3><span className="evidence-state collecting">Provider ledger</span></div><dl><div><dt>Provider-recorded calls</dt><dd>{formatNumber(currentMetric?.provider_recorded_calls ?? 0)}</dd></div><div><dt>Qualified calls</dt><dd>{formatNumber(currentMetric?.provider_confirmed_calls ?? 0)}</dd></div><div><dt>Pending review</dt><dd>{formatNumber(currentMetric?.provider_pending_calls ?? 0)}</dd></div><div><dt>Unsuccessful</dt><dd>{formatNumber(currentMetric?.provider_unsuccessful_calls ?? 0)}</dd></div></dl><p>Only Marketcall postbacks count as calls. Phone-button clicks and `tel:` handoffs remain internal diagnostics. Shared-number calls without exact attribution appear only in the portfolio total.</p></div>}
