...
Marketcall remains disabled. The dormant runtime now supports a domain-specific approved campaign, a hidden E.164 call destination revealed only after a recorded CTA click, and authenticated idempotent postbacks that exclude caller data. No provider offer, campaign, number, routing policy, or webhook secret is active in production. Campaign moderation, the public webhook hostname, disclosure review, provider-side postback configuration, and an explicit transition out of the measurement-only audit contract are still required before traffic can be sent. The traffic gate and the economic monetization gate are separate decisions: traffic evidence can justify a larger pilot, but cannot prove profitability.
