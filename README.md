...
Marketcall traffic remains disabled. The provider account now has one SEO-only campaign under moderation for each pilot domain: appliance repair, HVAC, and replacement-qualified roofing. Each campaign has its own free approved tracking number and advertising material under manager review. D1 records the corresponding offers as `draft` and campaigns as `submitted`; no offer, campaign, routing policy, webhook secret, click, conversion, or postback is active in production, and no tracking number appears in tenant HTML. Campaign and material approval, the public webhook hostname, provider-side postback configuration, and an explicit transition out of the measurement-only audit contract are still required before traffic can be sent. The traffic gate and the economic monetization gate are separate decisions: traffic evidence can justify a larger pilot, but cannot prove profitability.
