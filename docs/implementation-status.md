...
| Marketcall monetization | Economic pilot active on four exact placements | Three provider-approved SEO campaigns serve the original domains with dedicated DIDs; `anamechanical.com` explicitly shares the approved HVAC DID. User-facing call totals come only from provider postbacks and separate pending, qualified, and unsuccessful outcomes. CTA handoffs remain internal diagnostics. All placements use tracked click-to-call handoff, a public secret-path webhook, idempotent conversion projection, and an exact active-route audit contract | Monitor route/provider drift, rejected postbacks, qualified calls, and settled payouts before adding offers |
