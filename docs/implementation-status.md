...
- Exact-session v4 boundary: `2026-08-12 UTC`; v2 used a high-cardinality per-browser index and v3's single per-domain index still sampled under bursts, so earlier results remain latest-day context rather than being reported as zero exact uniques.
...
| Cloudflare zone assignments | Implemented for the pilot | Each domain row stores its unique zone ID, exact assigned nameserver pair, and live-verification timestamp; `pnpm sync:cloudflare-zones` requires Cloudflare API and control-API readback | Add every approved zone to the seed, then sync before registrar delegation |
...
| Marketcall monetization | Economic pilot active on four exact placements | Three provider-approved SEO campaigns serve the original domains with dedicated DIDs; `anamechanical.com` explicitly shares the approved HVAC DID. User-facing call requests require a measurement-eligible U.S. phone handoff with a valid first-party visitor-cookie hash. Marketcall records remain a separate pending, qualified, and unsuccessful outcome ledger. All placements use tracked click-to-call handoff, a public secret-path webhook, idempotent conversion projection, and an exact active-route audit contract | Monitor route/provider drift, rejected postbacks, qualified calls, and settled payouts before adding offers |
| Profitability | Measuring | Marketcall-recorded calls and qualified outcomes are provider-backed. Dedicated-DID conversions retain domain attribution; shared-HVAC-DID conversions remain honestly unattributed unless an exact click ID is supplied | Evaluate accepted revenue per valid session after the seven-day provider hold period settles |
