...
The admin exposes an evidence status, not an automatic expansion command. The boundary remains `2026-08-05`; failed or sampled days remain visible and are never rewritten as good evidence. `EXACT_SESSION_MIN_DATE` marks the first full UTC day produced entirely by the isolated session stream. A review requires at least 14 decision-grade days on or after that date, where the same day has complete rollup coverage, verified telemetry canaries, and an unsampled exact-session query. Fresh exact-release readiness and at least 95% scheduled health coverage/readiness for every published pilot tenant remain hard prerequisites. A transient bad day delays the review instead of permanently poisoning every later day:
