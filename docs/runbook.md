...
For a deterministic, telemetry-safe portfolio readback, run `pnpm audit:pilot`. It compares the exact pilot allowlist with the control-plane inventory, validates source eligibility, resolves every public `/readyz` response against the active release ID, checks stored freshness and scheduled reliability, and reports evidence-gate state. It exits with code `2` only for an operational problem; `observation_window` and `qualified_sessions` remain decision outcomes. The audit never requests tenant HTML and therefore creates no visitor events.
